...
var splitByWords = function (text) {
    //var removedPunctuation = text.replace(/[\.,\/#!$%\^&\*;:{}=\-_`~()]/g," ")
    var removedPunctuation = text.replace(/[\[\]\.,\/#!$%\^&\*;:{}=\-_`~()]/g, " ")
    var wordsArray = removedPunctuation.split(/\s+/);
    return wordsArray;
}
...
module.exports = configArg => {
    ...
    var fromMarkdownFile = function (filePath, encoding) {
        ...
        var result = fromMarkdown({ path: filePath, text: content });
        return result;
    }
    ...
    var fromHtmlFile = function (filePath, encoding) {
        ...
        console.log("read file - " + filePath);
        var content = fs.readFileSync(filePath, enc);
        var result = fromHtml({ path: filePath, text: content });
        return result;
    }
